...
export default function NewAd({ session, onBack, onPublished }) {
  ...
  const handleSubmit = async () => {
    ...
    // Moderazione AI prima della pubblicazione
    try {
      const modRes = await fetch("/api/moderate-ad", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: form.title,
      ...
      if (!modData.approved) {
        setError(`Annuncio non pubblicabile: ${modData.reason}`);
        setLoading(false);
        return;
      }
    } catch {
      // Se la moderazione fallisce per errore tecnico, si procede comunque
    }
